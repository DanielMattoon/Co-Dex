...
function CatalogTile({
  ...
  return (
    ...
    >
      <div
        className="h-11 w-8 rounded-sm border-2 border-slate-900"
        style={{ backgroundColor: color, boxShadow: `inset 0 0 0 3px rgba(0,0,0,0.25)` }}
      />
      <span className="text-[10px] leading-tight text-slate-200">{item.name}</span>
...
export function CollectionShelf() {
  ...
  // Chronological order, filtered by console. The catalog itself is
  // mainline-only for now (see collectibles.ts) — a flat, evenly-spaced
  // grid rather than the franchise-grouped/slide-open layout.
  const scopedCatalog = useMemo(() => {
    return [...catalog]
      .filter((c) => consoleFilter === 'all' || c.platform === consoleFilter)
      .sort((a, b) => a.release_order - b.release_order);
  }, [catalog, consoleFilter]);
  ...
  return (
    <div className="flex h-full flex-col gap-2 text-xs">
      ...
      <div className="flex-1 overflow-y-auto rounded-lg border border-slate-700 bg-slate-800/40 p-2">
        <div
          className="grid auto-rows-min justify-center gap-1.5"
        ...
        >
          ...
          {Array.from({ length: (SHELF_COLUMNS - (visibleCatalog.length % SHELF_COLUMNS)) % SHELF_COLUMNS }).map((_, i) => (
